...
public class GamePanel extends JPanel {
	...
	public void drawPanel() {
		for (int i = 0; i < bwMatrix.length; i++) { // --------> �̰� reset������ �ʱ�ȭ�Ǵ��� Ȯ�� �ʿ�
				...
				for (int i = 0; i < 20; i++) {
					for (int j = 0; j < 20; j++) {
						if (ellipse[i][j].contains(e.getPoint())) {
							for (int j2 = 0; j2 < Memory.points.size(); j2++) { // �ߺ� �Ǵ�
								if (Memory.points.get(j2).i == i && Memory.points.get(j2).j == j) {
									return;
								}
							}
							Color color;
							if (Memory.points.size() - Frame.blockCount == 0) { // �ٵϵ� ���� ����
//								color = Color.BLACK;
//								bwMatrix[i][j] = 1;
								if(Frame.swap) {
									color = Color.WHITE;
...
//								bwMatrix[i][j] = 2;
							...
							if (Memory.points.size() < Frame.blockCount) {// ���ÿ� �ֱ�
							...
							result = winDetection.detectWinner(i, j);// �����ϱ�
				...
				System.out.println("�� ���� : " + Memory.points.size());
				System.out.println("---------------------------------------------------");
				repaint();
